...
def setup_logging():
    """设置日志"""
    log_file = os.path.join(os.path.dirname(__file__), 'q4_solver.log')
    logging.basicConfig(
        level=logging.INFO,
...
class ParticleSwarmOptimizer:
    def __init__(self, objective_func, bounds, fixed_fy1_params, num_particles=90, max_iter=10000,
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("Q4 多无人机优化 - 开始")
    logger.info("=" * 60)

    start_total = time.time()

    # 1. 生成目标采样点
    logger.info("Step 1: 生成目标采样点...")
    target_samples = generate_ultra_dense_samples(real_target)
    logger.info(f"目标采样点数量: {len(target_samples):,}")
    ...
    # 5. 高精度验证最优解
    logger.info("Step 3: 验证最优解...")
    verify_fitness = fitness_function(best_params, target_samples)
    logger.info(f"优化阶段总遮蔽时长: {best_fitness:.4f} s")
    logger.info(f"验证阶段总遮蔽时长: {verify_fitness:.4f} s")

    # 6. 解析结果并保存Excel
    logger.info("Step 4: 解析策略并保存结果...")
    strategy_list = parse_best_params(best_params)
    result_df = save_to_excel(strategy_list, verify_fitness)

    # 7. 输出关键结果
    logger.info("=" * 80)
    logger.info("【优化结果汇总：FY1固定第二问参数，FY2/FY3优化】")
    logger.info(f"总有效遮蔽时长: {verify_fitness:.4f} s")
    logger.info("各无人机策略详情:")
    for i, strategy in enumerate(strategy_list):
        logger.info(f"{i + 1}. {strategy['无人机编号']}:")
        logger.info(f"   飞行方向: {strategy['飞行方向角(°)']}° | 速度: {strategy['飞行速度(m/s)']} m/s")
        logger.info(f"   投放延迟: {strategy['投放延迟(s)']}s | 起爆延迟: {strategy['起爆延迟(s)']}s")
        logger.info(f"   起爆点: ({strategy['起爆点X(m)']}, {strategy['起爆点Y(m)']}, {strategy['起爆点Z(m)']}) m")
        logger.info(f"   烟幕有效时段: [{strategy['起爆时刻(s)']}s, {strategy['烟幕失效时刻(s)']}s]")
        if i == 0:  # 标记FY1为固定参数
            logger.info(f"   注：该无人机参数为第二问固定结果，未参与本次优化")
    logger.info("=" * 80)

    # 8. 计算总耗时
    total_time = time.time() - start_total
    logger.info(f"程序总耗时: {total_time:.2f} 秒")
    logger.info("Q4 多无人机优化完成!")
